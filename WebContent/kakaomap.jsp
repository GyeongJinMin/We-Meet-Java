...
<body>
	...
	<script type="text/javascript"
		src="//dapi.kakao.com/v2/maps/sdk.js?appkey=	8db9dff95416858f6139281500bb3bd3&libraries=services"></script>
	<script>
		...
		function placesSearchCB(data, status, pagination) {
			if (status === kakao.maps.services.Status.OK) {
				shortest = Math.pow((data[0].y - centerx), 2)
						+ Math.pow((data[0].x - centery), 2);
				for (var i = 0; i < data.length; i++) {
					if (shortest > (Math.pow((data[i].y - centerx), 2) + Math
							.pow((data[i].x - centery), 2))) {
						shortestNum = i;
						shortest = Math.pow((data[i].y - centerx), 2)
								+ Math.pow((data[i].x - centery), 2);
					}
				}
				...
				window.sendMessage
						.sendPoint(finalLat, finalLong, shortest_Name);
		...
		for (var i = 0; i < x.length; i++) {
			...
			var linePath = [ new kakao.maps.LatLng(x[i], y[i]),
